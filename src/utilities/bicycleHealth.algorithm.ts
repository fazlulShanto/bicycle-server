...
const damageCalculateForOnePart = (
    ...
    const damage =
        Math.E **
        ((dependencyPart.roadConditionFactor *
            (1.2 * unpavedDistance + pavedDistance)) /
            k);
    if (dependencyPart.dependentPartId === "") {
        ...
        const dependencySubpart = bicycleSubParts.filter((bicycleSubPart) => {
            return (
                String(bicycleSubPart.subpart._id) ===
                dependencyPart.dependentPartId
            );
        });

        let CoefficientOfDamageOfFactorComponent = 1;
        if (
            dependencySubpart[0].health < 70 &&
            dependencySubpart[0].health >= 40
        ) {
            CoefficientOfDamageOfFactorComponent = 1.2;
        } else if (
            dependencySubpart[0].health < 40 &&
            dependencySubpart[0].health >= 20
        ) {
        ...
        subpart.health = Math.max(
            100 - CoefficientOfDamageOfFactorComponent * damage,
            0
        );
    }
};
...
const calculatePartsHealth = (
    ...
    bicycleDependency.forEach((dependencyPart) => {
        ...
        const totalPavedDistance =
            totalDailyCommutedPavedDistance +
            totalRecreationalCommutedPavedDistance;
        const totalUnpavedDistance =
            totalDailyCommutedUnpavedDistance +
            totalRecreationalCommutedUnpavedDistance;
...
const getDistance = (
    ...
    return (
        way *
        moment().diff(lastMaintainedDate, "week") *
        rodeDay *
        rodeDistance *
        rideFactor
    );
    ...
    allBicycle &&
        allBicycle.forEach((bicycle) => {
            ...
            if (bicycle.isRevised) {
                bicycle.revisionMonth &&
                    (lastRevisionMonth = bicycle.revisionMonth);
                bicycle.revisionYear &&
                    (lastRevisionYear = bicycle.revisionYear);
            }
            ...
            bicycle.totalHealth =
                totalSubpartHealth / bicycle?.bicycleParts!?.length;
